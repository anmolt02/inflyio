...
// ─── Nav definitions ──────────────────────────────────────────────────────────
// Only routes that actually exist are in here. No 404s.
const MAIN_NAV = [
...
const PLATFORM_NAV_LIVE = [
  { label: "YouTube", href: "/dashboard", icon: "▶", iconColor: "#FF4444", badge: "Live", badgeType: "live" },
];

// These render as styled divs — not Links — so they can never 404
const PLATFORM_NAV_SOON = [
  { label: "Instagram",   icon: "◉", iconColor: "#E1306C" },
  { label: "X / Twitter", icon: "✕", iconColor: "#60A5FA" },
];
...
function NavBadge({ type, label }: { type: string; label: string }) {
  ...
  return (
    <span style={{ marginLeft: "auto", fontSize: "9px", padding: "2px 6px", borderRadius: "4px", fontWeight: 500, background: s.bg, color: s.color, fontFamily: "'DM Mono',monospace" }}>
      {s.text}
    </span>
  );
}

function NavItem({ href, label, icon, iconColor, badge, badgeType, active, onClick }: {
  ...
  return (
    <Link href={href} style={{ textDecoration: "none" }} onClick={onClick}>
      <div
        style={{ display: "flex", alignItems: "center", gap: "11px", padding: "10px 12px", margin: "0 8px", borderRadius: "9px", background: active ? "rgba(96,165,250,0.10)" : "transparent", border: `1px solid ${active ? "rgba(96,165,250,0.22)" : "transparent"}`, color: active ? "#60A5FA" : "#64748B", fontSize: "13px", fontFamily: "'DM Mono',monospace", cursor: "pointer", transition: "background .15s,color .15s" }}
...
// Non-clickable "coming soon" platform item — never produces a 404
function SoonItem({ label, icon, iconColor }: { label: string; icon: string; iconColor?: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "11px", padding: "10px 12px", margin: "0 8px", borderRadius: "9px", color: "#334155", fontSize: "13px", fontFamily: "'DM Mono',monospace", cursor: "default", userSelect: "none" }}>
      <span style={{ width: "16px", height: "16px", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", color: iconColor ? `${iconColor}55` : "#334155" }}>
        {icon}
      </span>
      {label}
      <NavBadge type="soon" label="Soon" />
    </div>
  );
}

function SectionLabel({ text }: { text: string }) {
  return <div style={{ fontSize: "9px", color: "#334155", letterSpacing: ".12em", textTransform: "uppercase", padding: "10px 20px 5px", fontFamily: "'DM Mono',monospace" }}>{text}</div>;
}
...
function SidebarContent({ user, isActive, onNavClick }: {
  ...
  return (
    <>
      ...
      {PLATFORM_NAV_LIVE.map(n => <NavItem key={n.href} {...n} active={isActive(n.href)} onClick={onNavClick} />)}
      {PLATFORM_NAV_SOON.map(n => <SoonItem key={n.label} {...n} />)}
      <Divider />
      ...
      <div style={{ marginTop: "auto", padding: "16px 10px 0" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "9px", padding: "10px 12px", borderRadius: "9px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <div style={{ overflow: "hidden" }}>
            <div style={{ fontSize: "11px", color: "#F1F5F9", fontFamily: "'Syne',sans-serif", fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{displayName}</div>
            <div style={{ fontSize: "9px", color: "#334155", marginTop: "1px" }}>{user ? "Free Plan" : "Not signed in"}</div>
          </div>
        </div>
        {user && (
          <button
...
export default function Sidebar() {
  ...
  return (
    <>
      <style>{`
        @media(min-width:768px){ .sb-desk{display:flex!important} .mob-bar{display:none!important} }
        @media(max-width:767px){ .sb-desk{display:none!important} .mob-bar{display:flex!important} }
      `}</style>

      {/* Desktop sidebar */}
      <aside className="sb-desk" style={{ width: "220px", flexShrink: 0, background: "#0A0B0F", borderRight: "1px solid rgba(255,255,255,0.055)", flexDirection: "column", padding: "20px 0", position: "sticky", top: 0, height: "100vh", overflowY: "auto" }}>
        <SidebarContent user={user} isActive={isActive} />
      </aside>

      {/* Mobile top bar */}
      <div className="mob-bar" style={{ display: "none", position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, height: "52px", background: "#0A0B0F", borderBottom: "1px solid rgba(255,255,255,0.07)", alignItems: "center", justifyContent: "space-between", padding: "0 16px" }}>
        ...
        <button onClick={() => setDrawerOpen(!drawerOpen)} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "7px", padding: "6px 10px", cursor: "pointer", color: "#94A3B8", fontSize: "16px", lineHeight: 1 }}>
          {drawerOpen ? "✕" : "☰"}
        </button>
      </div>

      {/* Mobile overlay */}
      {drawerOpen && <div style={{ position: "fixed", inset: 0, zIndex: 99, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }} onClick={() => setDrawerOpen(false)} />}

      {/* Mobile drawer */}
      <div style={{ position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 100, width: "260px", background: "#0A0B0F", borderRight: "1px solid rgba(255,255,255,0.07)", display: "flex", flexDirection: "column", padding: "20px 0", overflowY: "auto", transform: drawerOpen ? "translateX(0)" : "translateX(-100%)", transition: "transform .28s cubic-bezier(.4,0,.2,1)" }}>
