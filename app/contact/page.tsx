...
const DISPLAY_EMAIL = "heyinflyio@gmail.com";
      ...
      <div className="contact-pad" style={{ padding: "32px 36px", maxWidth: "820px", fontFamily: "'DM Mono',monospace", color: C.textPri }}>
        ...
        <div className="f1" style={{ marginBottom: "36px" }}>
          ...
          <h1 style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(1.4rem,4vw,2rem)", letterSpacing: "-.025em", margin: 0 }}>
            Contact <span style={{ background: `linear-gradient(90deg,${C.blue},${C.cyan})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Us</span>
          </h1>
        ...
        <div className="contact-layout">
          <div className="f2" style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "16px", padding: "28px" }}>
            {status === "sent" ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 20px", gap: "14px", textAlign: "center" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: "12px", color: C.textSec, lineHeight: 1.7 }}>We'll get back to you within 24 hours.</div>
                </div>
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
                ...
                {status === "error" && (
                  <div style={{ padding: "10px 14px", background: "rgba(248,113,113,.07)", border: "1px solid rgba(248,113,113,.2)", borderRadius: "8px", fontSize: "12px", color: C.red }}>
                    ⚠ {errMsg} — or email us at <a href={`mailto:${DISPLAY_EMAIL}`} style={{ color: C.blue }}>{DISPLAY_EMAIL}</a>
                  </div>
                )}
                <button type="submit" disabled={status === "sending"} className="sbtn"
                  style={{ padding: "12px 24px", borderRadius: "10px", background: `linear-gradient(135deg,${C.blue},#3B82F6)`, border: "none", color: "#07080C", fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: "13px", cursor: status === "sending" ? "not-allowed" : "pointer", opacity: status === "sending" ? 0.65 : 1, alignSelf: "flex-start", display: "flex", alignItems: "center", gap: "8px" }}>
                  {status === "sending"
                    ? <><span style={{ width: "13px", height: "13px", border: "2px solid rgba(0,0,0,.25)", borderTopColor: "#07080C", borderRadius: "50%", display: "inline-block", animation: "spin .7s linear infinite" }} />Sending…</>
                    : "Send Message →"}
                </button>
              </form>
            )}
          ...
          <div className="f3" style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            <div style={{ background: C.blueDim, border: `1px solid ${C.blueBorder}`, borderRadius: "14px", padding: "20px", boxShadow: C.blueGlow }}>
              ...
              <a href={`mailto:${DISPLAY_EMAIL}`} style={{ display: "block", fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: "14px", color: C.textPri, textDecoration: "none", marginBottom: "4px" }}>{DISPLAY_EMAIL}</a>
